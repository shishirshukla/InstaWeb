...
NextPost.Account = function()
{
    ...
    $(document).ajaxComplete(function(event, xhr, settings) {
        ...
        if (xhr.responseJSON.hasOwnProperty("changes_saved") &&
            ...
            $form.find(".js-checkpoint, .js-2fa").addClass("none");
        ...
        if (xhr.responseJSON.hasOwnProperty("checkpoint_required") &&
            xhr.responseJSON.checkpoint_required) {
            verification_type = "checkpoint";
        }
        ...
        $form.find(".js-login, .js-checkpoint, .js-2fa").addClass("none");
        $form.find(".js-login, .js-checkpoint, .js-2fa").find(":input").prop("disabled", true);
        ...
        if (verification_type == "2fa") {
            ...
            // Update login form state to checkpoint
            $form.find(":input[name='action']").val("checkpoint");

            $form.find(":input[name='checkpointid']").val(xhr.responseJSON.identifier)
            $form.find(".js-checkpoint").removeClass("none")
            $form.find(".js-checkpoint :input").prop("disabled", false);
        }
    });


    // Resend Security Code
    // Compatible for both of 2FA and Checkpoint
    var resend_count = 0;
    $form.find(".resend-btn").on("click", function() {
        ...
        if ($parent.length != 1) {
            $parent = _this.parents(".js-checkpoint");
        }
        var type = $parent.hasClass('js-2fa') ? "2fa" : "checkpoint";
        ...
        $.ajax({
            ...
            dataType: 'jsonp',
            data: {
                action: type == "2fa" ? "resend-2fa" : "resend-checkpoint",
                id: type == "2fa" ? $form.find(":input[name='2faid']").val() : $form.find(":input[name='checkpointid']").val()
            },
            error: function() {
            ...
            success: function(resp) {
                ...
                if (resp.result == 1) {
                    if (type == "2fa") {
                        ...
                        $form.find(":input[name='checkpointid']").val(resp.identifier);
                    }
